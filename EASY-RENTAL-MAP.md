...
- **Supabase migration:**
  ...
  - DB password: [REDACTED — never store secrets in this repo; rotate if previously exposed]
- **Vercel migration:**
...
```
postgresql://postgres.qedlpnkbjgvgibhufpiq:[REDACTED]@aws-1-us-east-1.pooler.supabase.com:6543/postgres
```
...
### 2026-04-29 (Session 59) — CC — Email-first CTAs + Listings logo fix

**What happened:**
- Bill prefers email as contact; phone was leading everywhere. Swapped all public CTAs: hero ("Email us"), Landing Step 3 ("Email us"), Owners (email button first, call second), PropertyPanel contact (email first, call second).
- Listings page h1 had `[logo] Homes` — logo duplicated the nav brand. Removed img from h1; heading is now plain "Homes".
- NavBar: added `useLocation` + conditional `navbar__page-label` span. Shows "— Homes" next to "Easy Rental" brand when on `/listings`. Other pages unaffected.
- Committed `9312d5c`, pushed to `bill` + `origin`, `bash scripts/deploy.sh public` — all smoke tests green.

**Git:** Branch `master` | Commit `9312d5c` | Pushed to both remotes | Uncommitted: `EASY-RENTAL-MAP.md` (this entry); untracked: longstanding env/asset residue.
